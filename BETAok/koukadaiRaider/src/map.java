	...
	public static void map() {
		...
		for(int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				...
				roomArray[i][j].status = random.nextInt(8)+1;
				//possible exits
				for(int v = 0; v < 4; v++) {
				...
				if (roomArray[i][j].cont.equals("Loot")&& (f >= 0)) {
					roomArray[i][j].takara = random.nextInt(3) + 1;
					//System.out.print(" Loot:"+roomArray[i][j].takara);
					f--;
				} else if (roomArray[i][j].cont.equals("enemy") && (d != 2)){
					...
					roomArray[i][j].cont = "boss";
					roomArray[i][j].boss = 1;
					roomArray[i][j].takara = 4;
